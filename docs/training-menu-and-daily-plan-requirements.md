...
> リカバリー、日別計画、`restDates` 廃止に関する最新仕様は `docs/recovery-menu-requirements.md` を正本とする。本書中の休息専用計画に関する記述は廃止済み。
...
- 一時セットは自動削除しないこと。
- MCPで論理キャンセルされた一時セットは通常のメニュー一覧へ混在させず、メニュー画面の折りたたみ式管理欄から確認・物理削除できること。
- 論理キャンセル済みセットの物理削除前に、監査用のセット内容は復元できなくなる一方、確定済みの実施履歴は残ることを確認すること。
