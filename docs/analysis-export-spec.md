...
schemaVersion 2では、メニューと実施履歴に重量換算情報を含める。schemaVersion 3では、Daily記録へ睡眠、疲労、やる気、筋肉痛、痛み、安静時心拍数、自由記述の食事内容・栄養メモを追加する。schemaVersion 7では、Daily記録へ `muscleMassKg` を追加する。schemaVersion 8では、Daily記録へ `aiCoachReview` と `aiCoachReviewedAtUtc` を追加する。
