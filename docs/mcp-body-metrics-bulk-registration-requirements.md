...
- MCP公開schemaでリクエスト全体を拒否する検証と、MCP Lambdaがレコード別結果を返す検証を分離する。
...
- GatewayがLambda呼び出し前に配列要素を厳格検証する場合でも、レコード別エラーが全体のschemaエラーへ変わらないよう公開schemaの制約範囲を調整する。レコード内容の最終検証はLambdaを正とする。
...
19. Gatewayのschema検証によって、レコード単位で返すべき不正値や不明プロパティがリクエスト全体のエラーへ変換されない。
