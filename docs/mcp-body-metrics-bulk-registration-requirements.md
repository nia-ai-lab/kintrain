...
チャットやMCPクライアントから、複数日分の体重・体脂肪率・筋肉量を1回のMCPツール呼び出しで登録できるようにする。
...
- MCPツール `save_body_metrics` は、1回の呼び出しで1日分を部分更新する。
- `save_body_metrics` は `date` と、`bodyWeightKg`、`bodyFatPercent`、`muscleMassKg` の少なくとも1つを必須とする。測定時刻は任意とする。
...
| `bodyWeightKg` | number | 条件付き | `(0, 500]` kg、小数2桁まで。体組成値の少なくとも1つを必須 |
| `bodyFatPercent` | number | 条件付き | `[0, 100]` %、小数2桁まで。体組成値の少なくとも1つを必須 |
| `muscleMassKg` | number | 条件付き | `(0, 500]` kg、小数2桁まで。同日の体重を超えない。体組成値の少なくとも1つを必須 |
...
- `bodyWeightKg` と `muscleMassKg` は `(0, 500]` kg、`bodyFatPercent` は `[0, 100]` %、いずれも小数2桁までとする。
- 各レコードは `bodyWeightKg`、`bodyFatPercent`、`muscleMassKg` の少なくとも1つを含むこと。
- 入力値と既存値を合わせた同日の最終状態で `muscleMassKg <= bodyWeightKg` を満たすこと。
...
- 配列要素がオブジェクトでない、または `date`、体重、体脂肪率、筋肉量、測定時刻のプリミティブ型が異なる場合は、Gatewayのschemaエラーとしてリクエスト全体を400で拒否できる。
...
- JWT、Authorizationヘッダー、Cognito属性、体重・体脂肪率・筋肉量の全入力配列を通常ログへ出力しない。
...
体重・体脂肪率・筋肉量そのものをメトリクスのdimensionやログへ含めない。
...
| ID | 判断事項 | 選択肢 | 決定 | 影響 |
|---|---|---|---|---|
| D-01 | 一部項目だけの登録を許すか | A: 全体組成値を必須 / B: 少なくとも1項目を必須 | **B**。過去データへ筋肉量だけを追記でき、入力欠損を捏造せず登録できる | schema、既存値保持、受け入れテスト |
| D-02 | 測定時刻を必須にするか | A: 全行必須 / B: 任意 / C: 未指定時に固定時刻を補完 | **B**。不明な時刻を作らない。指定がなければ既存値を保持し、新規日は時刻なし | 単件・バッチ共通、Daily表示 |
